...
const receptionistsData = [
  { id: 'RC-201', name: 'Alice Johnson', shift: 'Morning (08:00 - 16:00)', phone: '+1 234-567-8910', email: 'alice@clinic.com', status: 'Active' },
  { id: 'RC-202', name: 'Bob Williams', shift: 'Evening (16:00 - 00:00)', phone: '+1 234-567-8911', email: 'bob@clinic.com', status: 'Active' },
];
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="shadow-sm border border-slate-100 !p-0 overflow-hidden">
        ...
        <Table headers={['Staff Info', 'Shift Schedule', 'Contact', 'Status', 'Actions']}>
          {receptionistsData.filter(r => r.name.toLowerCase().includes(searchTerm.toLowerCase())).map((staff) => (
            <TableRow key={staff.id}>
              ...
              <TableCell className="text-slate-600 font-medium">{staff.shift}</TableCell>
              <TableCell>
                <div className="text-sm text-slate-600 flex items-center gap-1"><Phone size={14} className="text-slate-400"/> {staff.phone}</div>
                <div className="text-sm text-slate-500 flex items-center gap-1 mt-1"><Mail size={14} className="text-slate-400"/> {staff.email}</div>
              </TableCell>
              <TableCell>
